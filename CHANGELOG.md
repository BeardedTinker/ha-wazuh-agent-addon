# Changelog

## [1.0.4](https://github.com/BeardedTinker/ha-wazuh-agent-addon/compare/v1.0.3...v1.0.4) (2026-03-04)


### Bug Fixes

* versioning ([6d08a59](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/6d08a5933cd7d8b8812aaf23bfc267f3f52fbc38))

## [1.0.3](https://github.com/BeardedTinker/ha-wazuh-agent-addon/compare/v1.0.2...v1.0.3) (2026-03-04)


### Bug Fixes

* fixing etc and machine-id in docker ([6190298](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/61902983bfd2130f0aee9032007f19e56646ed80))
* fixing etc and machine-id in docker ([705488d](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/705488d4d7295ca74428a8906923a11e274d1851))

## [1.0.2](https://github.com/BeardedTinker/ha-wazuh-agent-addon/compare/v1.0.1...v1.0.2) (2026-03-04)


### Bug Fixes

* try to get machine-id ([5a6fab8](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/5a6fab8363d054dd3b27e59ec0a07a2c7a423799))

## [1.0.1](https://github.com/BeardedTinker/ha-wazuh-agent-addon/compare/v1.0.0...v1.0.1) (2026-03-04)


### Bug Fixes

* change where changelog is located for HA Addon ([7602d7f](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/7602d7fd343bfc749042541a88915bb4555f9edc))
* content was overwritten ([0aba0ae](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/0aba0aefa7e0ef7d515fecda1667a90530e39452))
* Error with path ([2aace31](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/2aace3182f1d3ad69bdb3de742ffb6aa196b1827))
* trying to get journal access ([9905f8e](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/9905f8e8de756a94c13a928b9272683acb5695a2))
* trying to get journal access ([8e9290e](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/8e9290ed267af35ba0a0fa8ca8196285053ca341))
* Underscore is wrong ([fef40e0](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/fef40e04ee1699a1c2ddc9ed040f8db7c5d4b26e))

## [1.0.2](https://github.com/BeardedTinker/ha-wazuh-agent-addon/compare/ha-wazuh-agent-addon-v1.0.1...ha-wazuh-agent-addon-v1.0.2) (2026-03-04)
## 1.0.0 (2026-03-04)


### Bug Fixes

* content was overwritten ([0aba0ae](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/0aba0aefa7e0ef7d515fecda1667a90530e39452))
* Error with path ([2aace31](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/2aace3182f1d3ad69bdb3de742ffb6aa196b1827))
* Underscore is wrong ([fef40e0](https://github.com/BeardedTinker/ha-wazuh-agent-addon/commit/fef40e04ee1699a1c2ddc9ed040f8db7c5d4b26e))

## 0.3.0
- Production-grade release
- Persistent agent enrollment keys
- Auto-enrollment loop prevention
- Minimal security profile
- Journald-first log collection
- Configuration validation and hardening
- Documentation overhaul

## 0.2.x
- Initial public development
- Enrollment logic fixes
- Add-on storage handling
- Debug tooling

## 0.1.0
- Proof of concept
